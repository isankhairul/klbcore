...
class Select
{
    ...
    const HAVING = 'having';
    const ORDER = 'order';
    const WHERE_UNION = 'whereUnion';
    const GROUP_UNION = 'groupUnion';
    const HAVING_UNION = 'havingUnion';
    const ORDER_UNION = 'orderUnion';
    ...
    protected function _join( $type, $name, $cond, $cols, $schema = null )
    {
        if ( !in_array( $type, self::$_joinTypes ) && $type != self::FROM ) {
            /**
             * @see DbSelectException
             */
            throw new DbSelectException( "Invalid join type '$type'" );
        }

        if ( count( $this->_parts[self::UNION] ) ) {
            throw new DbSelectException( "Invalid use of table with " . self::SQL_UNION );
        }
        ...
        if ( !empty( $correlationName ) ) {
            if ( array_key_exists( $correlationName, $this->_parts[self::FROM] ) ) {
                 ...
                 * @see DbSelectException
                 */
                throw new DbSelectException( "You cannot define a correlation name '$correlationName' more than once" );
            }
    ...
    public function columns( $cols = '*', $correlationName = null )
    {
        ...
        if ( !array_key_exists( $correlationName, $this->_parts[self::FROM] ) ) {
             ...
             * @see DbSelectException
             */
            throw new DbSelectException( "No table has been specified for the FROM clause" );
        }
    ...
    public function union( $select = [], $type = self::SQL_UNION )
    {
        ...
        if ( !in_array( $type, self::$_unionTypes ) ) {
            throw new DbSelectException( "Invalid union type '{$type}'" );
        }
    ...
    {
        if ( !$condition = $this->_where( $cond, $value, $type, true ) ) {
            return $this;
        }
    ...
    protected function _where( $condition, $value = null, $type = null, $bool = true )
    {
        if ( $this->isUnion() ) {
            ...
            $cond = "";
            if ( $this->_parts[self::WHERE_UNION] ) {
                if ( $bool === true ) {
                    $cond = self::SQL_AND . ' ';
                } else {
                    $cond = self::SQL_OR . ' ';
            ...
            $this->_parts[self::WHERE_UNION][] = $cond . "($condition)";
    ...
    {
        return count( $this->_parts[self::UNION] ) > 0;
    }
    ...
    {
        if ( !$condition = $this->_where( $cond, $value, $type, false ) ) {
            return $this;
        }
        $this->_parts[self::WHERE][] = $condition;
    ...
    public function group( $spec )
    {
        ...
        $part = $this->isUnion() ? self::GROUP_UNION : self::GROUP;
    ...
    public function having( $cond, $value = null, $type = null )
    {
        ...
        $part = $this->isUnion() ? self::HAVING_UNION : self::HAVING;

        if ( $this->_parts[$part] ) {
            $this->_parts[$part][] = self::SQL_AND . " ($cond)";
        } else {
            $this->_parts[$part][] = "($cond)";
        }
    ...
    public function orHaving( $cond, $value = null, $type = null )
    {
        ...
        if ( $this->_parts[$part] ) {
            $this->_parts[$part][] = self::SQL_OR . " ($cond)";
        } else {
            $this->_parts[$part][] = "($cond)";
        }
    ...
    public function order( $spec )
    {
        ...
        foreach ( $spec as $val ) {
            if ( $val instanceof Expr ) {
                ...
                $this->_parts[$part][] = [ $val, $direction ];
            }
        }
    ...
    public function getPart( $part )
    {
        ...
        if ( !array_key_exists( $part, $this->_parts ) ) {
            throw new DbSelectException( "Invalid Select part '$part'" );
        }
    ...
    public function reset( $part = null )
    {
        if ( $part === null ) {
            $this->_parts = self::$partsInit;
        } else if ( array_key_exists( $part, self::$partsInit ) ) {
            if ( $part === self::UNION ) {
                $this->_parts[self::WHERE_UNION] = [];
                $this->_parts[self::GROUP_UNION] = [];
                $this->_parts[self::ORDER_UNION] = [];
            }
            $this->_parts[$part] = self::$partsInit[$part];
        }
    ...
    public function _joinUsing( $type, $name, $cond, $cols = '*', $schema = null )
    {
        if ( empty( $this->_parts[self::FROM] ) ) {
            throw new DbSelectException( "You can only perform a joinUsing after specifying a FROM table" );
        }
    ...
    protected function _quoteIdentifierAs( $ident, $alias = null, $auto = false, $as = ' AS ' )
    {
        ...
        } else if ( $ident instanceof Select ) {
     ...
     *
     * @return Select
     * @throws DbSelectException If an invalid method is called.
     */
        ...
        if ( preg_match( '/^join([a-zA-Z]*?)Using$/', $method, $matches ) ) {
            ...
            if ( $type ) {
                ...
                if ( !in_array( $type, self::$_joinTypes ) ) {
                    throw new DbSelectException( "Unrecognized method '$method()'" );
                }
                if ( in_array( $type, [ self::CROSS_JOIN, self::NATURAL_JOIN ] ) ) {
                    throw new DbSelectException( "Cannot perform a joinUsing with method '$method()'" );
                }
            } else {
        ...
        foreach ( $this->_parts[self::COLUMNS] as $columnEntry ) {
            ...
            if ( $column instanceof Expr ) {
                ...
                if ( empty( $correlationName ) ) {
                    ...
                    $columns[] = $this->quoteColumnAs( [ $correlationName, $column ], $alias, true );
                }
            }
        }
    ...
    protected function _renderUnion( $sql )
    {
        if ( $this->_parts[self::UNION] ) {
            ...
            foreach ( $this->_parts[self::UNION] as $cnt => $union ) {
                ...
                $sql .= $target;
                if ( $this->_parts[self::WHERE_UNION] ) {
                    $sql .= ' ' . self::SQL_WHERE . ' ' . implode( ' ', $this->_parts[self::WHERE_UNION] );
                }
                if ( count( $this->_parts[self::GROUP_UNION] ) > 0 ) {
                    $sql .= ' ' . self::SQL_GROUP_BY . ' ' . join( ', ', $this->_parts[self::GROUP_UNION] );
                }
                if ( count( $this->_parts[self::HAVING_UNION] ) > 0 ) {
                    $sql .= ' ' . self::SQL_HAVING . ' ' . implode( ' ', $this->_parts[self::HAVING_UNION] );
                }
                if ( $cnt < $parts - 1 ) {
                    $sql .= ' ' . $type . ' ';
                }
    ...
    protected function _renderLimitoffset( $sql )
    {
         ...
         */
        if ( $count > 0 ) {
            $sql = trim( $this->limit( $count, $offset ) );
        }
